...
class CreateOrEdit extends Component {
  ...
  getCrate = (crateId) => {
    if (crateId > 0) {
      this.props.getCrateById(crateId)
        ...
        .catch(error => {
          this.props.messageShow('Ocurrió un error al buscar los tipos de cajas. Por favor intente de nuevo.')
        })
    }
  }
  ...
  onSubmit = (event) => {
    ...
    this.props.messageShow('Guardando caja, por favor espere...')
    ...
    this.props.crateCreateOrUpdate(this.state.crate)
      .then(response => {
        ...
        if (response.data.errors && response.data.errors.length > 0) {
          ...
          this.props.messageShow('Caja guardada exitosamente.')
